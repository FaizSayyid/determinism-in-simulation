...
In Joshua Epstein's influential 2008 article "why model" Epstein discusses prediction:
...
In this example we have marginalised (integrated) over all possible values that the womb can take. Through integration, we have "removed" the variable womb from the conditioning set as we do not know the values that it takes, which in turn increases the uncertainty in our model. **The conditional dependance remains but it is now expressed as an average**. 

If the nature of this distribution is well understood we can explicity marginalise this variable out and different outcomes of the womb variable will recieve different weightings according to their probability under the womb distribution. In the much more likely case that we do not know that the womb variable exists we are implicity marginalising the womb variable away by modelling the height as a random variable, with an expectation (average) that is composed of the variables that we do know. This is of course, like all modelling, is a bit of a fudge.
